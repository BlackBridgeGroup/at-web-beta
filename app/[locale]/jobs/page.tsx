...
const T: Record<string, Content> = {
  de: {
    ...
    ctaT: 'Passt eine dieser Rollen zu dir?', ctaB: 'Registriere dich kostenlos — wir melden uns mit passenden Rollen. Die Gebühr zahlt der Arbeitgeber, niemals du.',
    ctaBook: 'Kostenlos registrieren', ctaProfile: 'Ratgeber lesen',
  ...
  cz: {
    ...
    ctaT: 'Sedí ti některá z těch rolí?', ctaB: 'Zaregistruj se zdarma — ozveme se s vhodnými rolemi. Poplatek platí zaměstnavatel, nikdy ty.',
    ctaBook: 'Registrovat se zdarma', ctaProfile: 'Číst rádce',
  ...
  en: {
    ...
    ctaT: 'Does one of these roles fit you?', ctaB: 'Register for free — we reach out with matching roles. The fee is paid by the employer, never by you.',
    ctaBook: 'Register for free', ctaProfile: 'Read the guides',
      ...
      <section style={{ background: 'var(--at-alpine-green)', color: '#fff', paddingBlock: 'var(--space-8)', textAlign: 'center' }}>
        <div className="at-container" style={{ maxWidth: 620 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            <Link href={`/${locale}/fragebogen`} className="at-btn" style={{ background: '#fff', color: 'var(--at-alpine-green)', fontWeight: 700 }}>{t.ctaBook}</Link>
            <Link href={`/${locale}/resources`} className="at-btn" style={{ background: 'transparent', color: '#fff', boxShadow: 'inset 0 0 0 1.5px rgba(255,255,255,0.5)' }}>{t.ctaProfile}</Link>
          </div>
        </div>
      </section>
    </SiteShell>
